...
function localDateString(date, timezone){
  const parts = Object.fromEntries(new Intl.DateTimeFormat('en-CA', { timeZone:timezone, year:'numeric', month:'2-digit', day:'2-digit' }).formatToParts(date).filter(p => p.type !== 'literal').map(p => [p.type, p.value]));
...
function nordPoolUrl(area, date){
  ...
  const deliveryDate = typeof date === 'string' ? date : localDateString(date, market.timezone);
  const params = new URLSearchParams({ market:'DayAhead', currency:'EUR', deliveryArea:market.nordpoolArea, date:deliveryDate });
...
async function fetchPrices(area){
  const market = MARKETS[normalizeArea(area)];
  const today = localDateString(new Date(), market.timezone);
...
function privacyPage(){ return new Response(PRIVACY_HTML, { headers:{ 'content-type':'text/html; charset=utf-8', 'cache-control':'public, max-age=3600' } }); }
...
function localizeStatic(){
  ...
  $('chartTitle').textContent = tr('chartTitle'); $('chartExplain').textContent = tr('chartExplain'); $('legendCheap').textContent = tr('cheap'); $('legendMiddle').textContent = tr('middle'); $('legendHigh').textContent = tr('expensive'); $('xLabel').textContent = tr('time'); $('sourceLine').textContent = tr('source'); $('legalFooter').innerHTML = tr('legal');
...
const PRIVACY_HTML = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ElektroPrice Privacy Policy</title><style>body{font-family:-apple-system,BlinkMacSystemFont,"SF Pro Text",Inter,sans-serif;background:#f3f7fb;color:#102033;margin:0;padding:28px;line-height:1.55}main{max-width:760px;margin:auto;background:#fff;border:1px solid #dce6f1;border-radius:24px;padding:24px}h1{letter-spacing:-.04em}a{color:#0b65d8}</style></head><body><main><h1>ElektroPrice Privacy Policy</h1><p><b>Owner:</b> SIA EuroVentro.</p><p>ElektroPrice shows live electricity market prices and appliance cost estimates. The app does not require an account, does not sell personal data, does not use App Tracking Transparency/IDFA, and does not include in-app purchases.</p><p>Basic technical logs may be processed by hosting and analytics infrastructure to keep the service reliable and secure. Price requests can include the selected market area and app/browser technical information. We do not use this to identify individual users.</p><p>Data source: Nord Pool day-ahead electricity market data. Final electricity bills depend on each user's supplier, network tariff, taxes and local fees.</p><p>Contact: <a href="mailto:support@euroventro.com">support@euroventro.com</a></p><p><a href="/">Back to ElektroPrice</a></p></main></body></html>`;
